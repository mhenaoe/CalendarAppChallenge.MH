...
class Calendar:
    ...
    def add_event(self, title: str, description: str, date_: date, start_at: time, end_at: time):
        ...
        if date_ not in self.days:
            ...
            event = Event(title, description, start_at, end_at)
    ...
    def find_available_slots(self, date_: date) -> list[time]:
        return [
            time(hour, 0)
            for hour in range(9, 18)  # De 9 AM a 5 PM
            if all(event.date != date_ or event.time != time(hour, 0) for event in self.events.values())


        ]

    def update_event(self, event_id: str, title: str, description: str, date_:
    date, start_at: time, end_at: time):
    ...
    def find_events(self, start_at: date, end_at: date) -> dict[date,list[Event]] | None:
